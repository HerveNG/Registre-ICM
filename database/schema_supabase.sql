...
create table if not exists public.journal_audit (
    ...
    action       text not null check (action in ('creation', 'modification', 'suppression', 'reinitialisation')),
    registre_id  uuid,
...
-- ------------------------------------------------------------
-- 13. PRÉSENCES & STATISTIQUES DES CULTES
-- ------------------------------------------------------------
-- Module indépendant du registre des baptêmes/mariages, pour suivre la
-- fréquentation des cultes. Réutilise role_utilisateur() (section 6) et
-- touch_updated_at() (section 4) — pas de nouvelle logique de rôle.
--
...
create table if not exists public.attendance_record (
    ...
    -- pour l'afficher (comme journaliser_registre() a dû le faire § 10).
    -- updated_by n'est renseigné que par une véritable modification —
    -- jamais à la création (voir app.py, même choix côté Flask).
    created_by       text,
    updated_by       text,
...
from (values
    ...
    ('Hommes — Enfants',      'fils_icm', 13, 17,   0),
...
-- ------------------------------------------------------------
-- 14. MODULE FILS — registre nominatif et suivi de présence individuelle
-- ------------------------------------------------------------
-- Distinct de la catégorie agrégée « Fils-ICM » du § 13 (un simple compteur
-- par tranche d'âge/sexe, sans identité) : ce module tient un registre
-- nominatif (un fils = une personne identifiée) et son historique de
-- présence individuelle, activité par activité. Les deux coexistent sans
-- lien entre eux.
--
-- fils_activite réutilise service_type (§ 13, déjà configurable dans
-- Paramètres) comme type d'activité plutôt qu'une deuxième table de
-- configuration — un même « Culte du dimanche » désigne le même événement
-- réel dans les deux modules.
...
create table if not exists public.fils_activite (
    ...
    service_type_id  uuid not null references public.service_type (id),
    nom              text,   -- libellé optionnel (ex. « Réunion spéciale »)
    created_at       timestamptz not null default now(),
...
-- RLS : lecture pour tout compte muni d'un rôle ; insertion/modification
-- ouvertes aux trois rôles ; suppression définitive réservée secretaire/pasteur
-- (même reclassification des droits que le reste de l'application, § 5/§ 7).
...
-- « Réunion des fils » — un seul type d'activité propre au module, ajouté
-- à la liste déjà partagée avec Présences (mêmes valeurs que côté Flask,
-- app.py : initialiser_donnees_fils()).
insert into public.service_type (nom, ordre_affichage)
...
-- ============================================================
-- 15. RÉINITIALISATION DU JOURNAL D'AUDIT (compte unique)
-- ============================================================
-- Le journal reste immuable pour tout le monde (§ 12.3 : aucune policy
-- update/delete, REVOKE UPDATE, DELETE ... FROM authenticated). Ici, une
-- SEULE exception, volontairement étroite : une fonction RPC qui vide
-- entièrement le journal, mais qui refuse d'agir pour tout compte autre
-- que celui dont l'e-mail figure dans son corps (vérifié via le jeton de
-- la personne connectée — une personne précise, pas un rôle "profils").
--
-- Comme journaliser_registre() (section 10) le fait déjà pour ÉCRIRE dans
-- ce même journal, cette fonction contourne le REVOKE via security
-- definer : elle s'exécute avec les droits de son propriétaire, qui lui
-- n'a jamais été privé de DELETE. Exposée en RPC à "authenticated"
-- (n'importe quel compte connecté peut l'appeler), c'est la vérification
-- interne de l'e-mail — pas le GRANT — qui réserve l'effet réel au seul
-- compte autorisé ; tout autre compte reçoit une erreur, sans que rien
-- ne soit modifié.
--
-- La réinitialisation laisse volontairement une trace : une unique ligne
-- "reinitialisation" est réécrite juste après la purge (qui, quand,
-- combien de lignes effacées) — sinon l'opération qui vide le journal
-- serait elle-même invisible dans le journal.
--
...
comment on function public.reinitialiser_journal_audit() is
  'Vide entièrement journal_audit puis y réécrit une seule ligne de trace — réservé à un compte unique désigné dans le corps de la fonction (§ 15).';
